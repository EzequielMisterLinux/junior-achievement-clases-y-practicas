    ...
    try {
        ...
        agregarProducto.innerHTML = `
            <form id="producto-form" class="max-w-sm mx-auto">
                ...
                <div class="mb-5">
                    <label for="dropzone-file" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Imagen del producto</label>
                    <div class="relative">
        ...
        const handleFile = (file) => {
            if (file && file.type.startsWith('image/')) {
                ...
                dropzoneFileInput.files = new DataTransfer().files;
                const fileList = new DataTransfer();
                fileList.items.add(file);
                dropzoneFileInput.files = fileList.files;
            }
        };
        ...
        const transformandoABooleano = (value) => {
        ...
        form.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                if (fileInput.files[0]) {
                    formData.set("imagen", fileInput.files[0]);
                }

                formData.set("precio", parseFloat(formData.get("precio")));

                const disponibilidadValue = document.getElementById("opcions").value;
       
              formData.set("disponibilidad", transformandoABooleano(disponibilidadValue));
