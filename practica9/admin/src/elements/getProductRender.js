...
const modal = new Modal($targetEl, options, instanceOptions);
    ...
    updateProduct.addEventListener("click", () => {
      ...
      updateById.addEventListener("click", async() => {
        ...
        let opctions = document.getElementById("opcionsUpdate")

        const transformandoAboolenano = () => {
          
            
            return opctions.value === "true"
          
        }

        let booleanDato = transformandoAboolenano()
  
        const newData = {
          nombre: nombre,
          descripcion : descripcion,
          precio: precio,
          disponibilidad : Boolean(booleanDato)
        }
  
        console.log(await updateProductById(item._id, newData));
